...
void SetFileInfoStrings(const CFileInfo &fileInfo,
    CFileInfoStrings &fileInfoStrings, int nMaxNameSize)
{
  farChar buffer[256];

  if (fileInfo.SizeIsDefined)
  {
    g_StartupInfo.m_FSF.sprintf(buffer, _F("%I64u "), fileInfo.Size);
    fileInfoStrings.Size = buffer;
    fileInfoStrings.Size += g_StartupInfo.GetMsgString(NMessageID::kOverwriteBytes);
  }
  else
  {
    fileInfoStrings.Size = _F("");
  }
...
NResult::EEnum Execute(const CFileInfo &oldFileInfo, const CFileInfo &newFileInfo)
{
  ...
  SetFileInfoStrings(oldFileInfo, oldFileInfoStrings, kXSize-14);
  SetFileInfoStrings(newFileInfo, newFileInfoStrings, kXSize-14);
  ...
  struct CInitDialogItem anInitItems[]={
    ...
    { DI_TEXT, 3, 3, 0, 0, false, false, DIF_BOXCOLOR|DIF_SEPARATOR, false, -1, _F(""), NULL  },  
    
    { DI_TEXT, 5, 4, 0, 0, false, false, 0, false, NMessageID::kOverwriteMessageWouldYouLike, NULL, NULL },

    { DI_TEXT, 7, 6, kXSize-7, 0, false, false, 0, false,  -1, oldFileInfoStrings.Name, NULL },
    ...
    { DI_TEXT, 7, 12, kXSize-7, 0, false, false, 0, false,  -1, newFileInfoStrings.Name, NULL },
    ...
    { DI_TEXT, 3, kYSize - 5, 0, 0, false, false, DIF_BOXCOLOR|DIF_SEPARATOR, false, -1, _F(""), NULL  },  
